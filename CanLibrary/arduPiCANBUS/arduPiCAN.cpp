 ...
 *  Version:	0.2
 *  Design:		David Gascon
 *  Implementation:		Luis Antonio Martin, Ahmad Saad, Ruben Martin
 */
...
//!*************************************************************
//!	Name: ON()													
//!	Description: Power ON embebed socket
//!	Param: void														
//!	Returns: none							
//!*************************************************************
void CAN::ON()
{
  ...
  digitalWriteSoft(SOCKET_PW,HIGH);
}
...
void CAN::OFF()
{
  //Power ON embebed socket
  pinMode(SOCKET_PW,OUTPUT); 
  digitalWriteSoft(SOCKET_PW,LOW);
}
...
bool CAN::begin(uint16_t speed)
{
	...
	switch(speed){
    	...
    	case 1000:
			...
			printf("Speed=1Mbps\n");
		#endif
			digitalWrite(CAN_CS,LOW); //Embebed Socket 	
            Utils.setCSSocket0(); //Multiprotocol v2.0
			...
			digitalWrite(CAN_CS,HIGH); //Embebed Socket		
            Utils.unsetCSSocket0(); //Multiprotocol v2.0
      	break;
...
{   
	...
	messageTx.data[1]= 0x01; //mode 1 for diagnostics mode 3 and no PID
